...
function List({ employees, handleEdit, isAdmin, handleLogin, token, setIsAdding }) {
    ...
    useEffect(() => {
        axios.post('https://bidgely-roster.herokuapp.com/api/profiles/all', { auth: token })
            .then(data => {
                setlist(data.data);
            })
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.value) {
                axios.post('https://bidgely-roster.herokuapp.com/api/profiles/delete', { auth: token, id })
    ...
    useEffect(() => {
        if (true) {
            axios.post('https://bidgely-roster.herokuapp.com/api/profiles/search', { auth: token, name: query })
                .then(data => {
                    const res = [];
